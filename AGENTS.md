...
- **Cline Lifecycle plugin** (`cline-lifecycle/`): a third, hand-authored TypeScript plugin exposing G1–G10 Agentic SDLC lifecycle governance as 8 tool calls: 4 forge-agnostic (`sdlc_init`/`sdlc_validate`/`sdlc_status`/`sdlc_decide`) plus 4 GitLab-specific (`sdlc_approve_from_gitlab`/`sdlc_approve_from_gitlab_mr`/`sdlc_link_intent_from_gitlab_issue`/`sdlc_link_requirements_from_gitlab_issue`, mirroring the GitLab-specific kernel subcommands `cadre-lifecycle-gitlab`'s skills document), wrapping the exact `bin/cadre sdlc <subcommand>` invocations the lifecycle plugins' skills already document for Claude Code/Codex. Requires the external `agentic-sdlc` kernel to be installed separately.
