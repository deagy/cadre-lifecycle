...
- **Cline Lifecycle plugin** (`cline-lifecycle/`): a third, hand-authored TypeScript plugin exposing G1–G10 Agentic SDLC lifecycle governance as 21 tool calls: 5 forge-agnostic (`sdlc_init`/`sdlc_validate`/`sdlc_plan`/`sdlc_status`/`sdlc_decide`) plus 16 forge-specific (7 GitLab, 7 GitHub, 2 shared across both forges), mirroring every forge-specific kernel subcommand `cadre-lifecycle-gitlab`'s and `cadre-lifecycle-github`'s skills document except `brief-pending-gates-*` (which just wraps `sdlc_status`, nothing new to mirror), wrapping the exact `bin/cadre sdlc <subcommand>` invocations the lifecycle plugins' skills already document for Claude Code/Codex. `sdlc_plan` wraps `agentic-sdlc plan`, referenced only in passing across forge-specific skills rather than given a numbered step of its own in any one of them. 10 of the 16 forge-specific tools wrap kernel subcommands that were missing from the `agentic-sdlc` version pinned when they were first added (traced to a kernel-side `VERSION`-drift bug, fixed upstream in `deagy/agentic-sdlc` v0.13.0); this repo's `provider.json` now pins `kernel_compatibility` to `[0.13.0, 1.0.0)` and all 10 are live-verified working — see `cline-lifecycle/README.md`. Requires the external `agentic-sdlc` kernel to be installed separately.
