...
`approve-from-github`/`approve-from-github-pr` (run `./bin/cadre sdlc --help`
...
## Step 3 — Which command: prepared evidence, or fetch from the PR?

Ask the human whether they already know the specific GitHub review's details
(review id, reviewer's login, commit SHA), or would rather you fetch the
approved review directly from the PR via the GitHub CLI (`gh`):

- **They have the review details already** (or you're recording evidence a
  colleague reported to you) → `approve-from-github` (Step 4a).
- **They just want to point at a PR and have you find the approved review**
  → `approve-from-github-pr`, which requires authenticated `gh` access and
  fails closed if it can't reach GitHub or find an approved review on that
  PR (Step 4b).

## Step 4a — approve-from-github (prepared evidence)

Ask for: the GitHub repository (`owner/repo`), the PR number, the review id,
the reviewer's GitHub login, and the commit SHA the review approved. Then
run:

```sh
./bin/cadre sdlc approve-from-github --root <path> --task-id <task-id> \
  --gate <G_id> --role <role> --repo <owner/repo> --pr <number> \
  --review-id <review-id> --reviewer-login <login> --commit-sha <sha> \
  [--decided-at <RFC3339>]
```

## Step 4b — approve-from-github-pr (fetch from the PR)

Ask for: the GitHub repository (`owner/repo`) and the PR number. Then run:

```sh
./bin/cadre sdlc approve-from-github-pr --root <path> --task-id <task-id> \
  --gate <G_id> --role <role> --repo <owner/repo> --pr <number> \
  [--reviewer-login <login>] [--commit-sha <sha>]
```

`--reviewer-login`/`--commit-sha` narrow which approved review counts, if the
PR has more than one and the human wants a specific one; otherwise omit them
and let the kernel select.

## Step 5 — Report the outcome

Parse the returned JSON yourself. Report the outcome in one plain sentence
("Recorded — Architecture is now approved, sourced from PR #42's review"),
