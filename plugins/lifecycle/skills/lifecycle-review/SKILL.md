...
`lifecycle-onboarding`'s one-time initial setup. The human you are talking to
...
reachability check as `lifecycle-onboarding`'s "Before you start"). If it's
...
the bare `agentic-sdlc` binary, exactly as `lifecycle-onboarding` does, so
...
`lifecycle-onboarding`'s Step 4 role table (e.g. "Are you deciding this as
...
reassign the authority (point back at `lifecycle-onboarding`'s Step 4
...
`lifecycle-onboarding`'s Step 9 translates `validate` blockers, for example:
