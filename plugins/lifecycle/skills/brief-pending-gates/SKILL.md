...
and point them at the `lifecycle-onboarding` skill (setup) or `cadre sdlc
...
exactly as `lifecycle-onboarding` and `lifecycle-review` do, so this suite's
...
`lifecycle-review`/`link-source-issue-*` already read) and, for each gate
...
`lifecycle-onboarding`/`lifecycle-review` already read). For each
